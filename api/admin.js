...
export const getAdminStats = async () => {
  ...
  try {
    const [users, investments, deposits, withdrawals, tickets, txVolume] =
      await Promise.all([
    ...
    const data = {
      ...
      activeInvestments: investments.data.filter((i) => i.status === "active")
        .length,
      totalInvestedValue: investments.data.reduce(
        (acc, curr) => acc + parseFloat(curr.amount),
        0,
      ),
      pendingDeposits: deposits.data.filter((d) => d.status === "pending")
        .length,
      totalDepositValue: deposits.data
        .filter((d) => d.status === "approved")
        .reduce((acc, curr) => acc + parseFloat(curr.amount), 0),
      pendingWithdrawals: withdrawals.data.filter((w) => w.status === "pending")
        .length,
      totalWithdrawalValue: withdrawals.data
        .filter((w) => w.status === "approved")
        .reduce((acc, curr) => acc + parseFloat(curr.amount), 0),
      openTickets: tickets.count,
      platformTransactionVolume: txVolume.data.reduce(
        (acc, curr) => acc + parseFloat(curr.amount),
        0,
      ),
    };
...
export const toggleUserAccess = async (userId, newStatus) => {
  ...
  try {
    ...
    await createAuditLog(
      auth.adminId,
      `${actionLabel} User Account #${userId}`,
      userId,
    );
