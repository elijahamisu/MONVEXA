...
export const createInvestment = async (planData) => {
  try {
    ...
    if (currentBalance < investAmount) {
      throw new Error(
        `Insufficient wallet balance. You need ₦${(investAmount - currentBalance).toLocaleString()} more.`,
      );
    }
    ...
    if (investAmount < planData.minLimit || investAmount > planData.maxLimit) {
      throw new Error(
        `Amount must be between ₦${planData.minLimit.toLocaleString()} and ₦${planData.maxLimit.toLocaleString()}.`,
      );
    }
...
export const distributeDailyEarnings = async () => {
  try {
    ...
    if (!activeInvs || activeInvs.length === 0)
      return formatResponse(true, "No active investments to process.");
    ...
    for (const inv of activeInvs) {
      ...
      const { data: wallet } = await supabase
        ...
        .single();
      const newBalance =
        parseFloat(wallet.balance) + parseFloat(inv.daily_profit);
...
export const getInvestmentStats = async () => {
  try {
    ...
    const stats = {
      ...
      averageInvestment:
        all.length > 0
          ? all.reduce((s, i) => s + parseFloat(i.amount), 0) / all.length
          : 0,
    };
