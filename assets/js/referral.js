...
const refreshReferralData = async () => {
  ...
  if (statsRes.success) {
    document.getElementById("totalRefs").innerText =
      statsRes.data.totalReferrals;
    document.getElementById("totalRefEarnings").innerText = formatCurrency(
      statsRes.data.totalEarnings,
    );

    // Month logic (calculated from local date filter if needed)
    const monthEl = document.getElementById("monthRefEarnings");
    if (monthEl)
      monthEl.innerText = formatCurrency(statsRes.data.totalEarnings); // Placeholder for monthly logic
  }
...
const renderReferralList = () => {
  ...
  const filtered = referralHistory.filter((u) => {
    ...
    const matchesStatus =
      filters.status === "all" ||
      (filters.status === "active" ? u.status === "active" : true);
...
const setupReferralRealtime = () => {
  ...
  subscribeToTable(
    "referral-reward-sync",
    "transactions",
    "INSERT",
    (payload) => {
      if (
        payload.new.user_id === currentUser.id &&
        payload.new.type === "referral"
      ) {
        refreshReferralData();
        notify(
          `Referral Commission Credited: ${formatCurrency(payload.new.amount)}`,
          "success",
        );
      }
    },
